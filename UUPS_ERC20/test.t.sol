...
contract Testing is Test {
    ...
    function setUp() external {
        _logic = new MyToken();
        _proxy = new Proxy(address(_logic));
        logic = ILogic(address(_proxy));
        proxy = IProxy(address(_proxy));
    ...
    function testInitialize() external {
        assertEq(address(this), logic.owner());
        assertEq(address(_logic), proxy._implementation());
        vm.expectRevert();
        logic.initialize("Gakarot","GAK$");
        assertEq(logic.balanceOf(ironman), 100e18);
        assertEq(logic.balanceOf(gakarot), 100e18);
    }
    ...
    function testUpgrade() external {
        MyToken one = new MyToken();
        logic.upgradeToAndCall(address(one), "");
        assertEq(address(one), proxy._implementation());
    }
