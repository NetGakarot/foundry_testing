...
interface ILendingPool {
    function getBorrowed(address asset) external view returns(uint256);
...
contract Staking is Ownable {
    ...
    constructor(address _aggr, address _lending) Ownable(msg.sender) {
    ...
    function changeLending(address _lending) external onlyOwner {
        lending = ILendingPool(_lending);
    }
...
function withdraw(address asset, uint256 amount) external {
    ...
    // Check if user has outstanding debt in LendingPool
    uint256 userDebt = lending.getBorrowed(asset);
        if (userDebt > 0) revert("Cannot withdraw: outstanding debt");
    ...
    function _updateLiquidityIndex(address asset) internal {
        ...
        uint256 rate = _getRatePerSecond(_apy(asset));
    ...
    function _apy(address asset) internal view returns(uint256) {
        uint256 totalBorrowed = lending.getBorrowed(asset);
        uint256 totalSupply = totalPool[asset];
        if (totalSupply == 0) return 0;
        uint256 utilizationRate = (totalBorrowed * RAY) / totalSupply;
        return (lending.getBorrowAPY(asset) * utilizationRate * (RAY - reserveFactor));
    }
}
